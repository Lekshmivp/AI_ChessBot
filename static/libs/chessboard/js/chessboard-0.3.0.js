...
function validFen(fen) {
  ...
  // cut off any move, castling, etc info from the end and only interested in position information
  fen = fen.replace(/ .+$/, '');
...
// convert FEN string to position object and returns false if the FEN string is invalid
...
// position object to FEN string and returns false if the obj is not a valid position object
...
function checkDeps() {
  ...
  // else it must be something that becomes a jQuery collection with size 1 ie: a single DOM node or jQuery object
  else {
...
// returns the square of the closest instance of piece returns false if no instance of piece is found in position
...
function beginDraggingPiece(source, piece, x, y) {
  // run their custom onDragStart function their custom onDragStart function can cancel drag start
...
function addEvents() {
  ...
  // IE doesn't like the events on the window object, but other browsers perform better that way
  if (isMSIE() === true) {
